defmodule AiChat.Prompts do
  ...
  @doc """
  Get prompts for a specific department and role.
  """
  def get_prompts_for_user(user) do
    from(p in Prompt,
      where: p.is_active == true and
             (^user.department_id in p.department_ids or p.department_ids == []) and
             (^user.role_id in p.role_ids or p.role_ids == [])
    )
    |> Repo.all()
  end
end
